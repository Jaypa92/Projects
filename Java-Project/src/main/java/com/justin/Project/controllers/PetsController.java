...
public class PetsController {
	...
	public String processQuiz(@ModelAttribute("pet")Quiz quiz) {
		...
		if(quiz.getAnswer().equals("extreme")) {
			None++;
		}
		if(quiz.getAnswer().equals("medium")) {	
			FishBird++;
		}
		if(quiz.getAnswer().equals("moderate")) {
			CatDog++;
		}
		if(quiz.getAnswer().equals("free")) {
			Any++;
		}
		if(quiz.getAnswer2().equals("yes")) {
			Any++;
		}
		if(quiz.getAnswer2().equals("very")) {
			CatDog++;
		}
		if(quiz.getAnswer2().equals("somewhat")) {
			FishBird++;		
		}
		if(quiz.getAnswer2().equals("no")) {
			None++;
		}
		if(None >=1) {
			return "redirect:/result/none";
		}
		if(FishBird >=1) {
			return "redirect:/result/fishBird";
		}
		if(CatDog >= 1) {
			return "redirect:/result/catDog";
		}
		if(Any >=1) {
			return "redirect:/result/any";
		}
		return "redirect:/about";
	}
	
	@GetMapping("/result/any")
	public String any() {
		return "any.jsp";
	}
	
	@GetMapping("/result/none")
	public String none() {
		return "none.jsp";
	}
	
	@GetMapping("/result/catDog")
	public String CatDog() {
		return "catDog.jsp";
	}
	
	@GetMapping("/result/fishBird")
	public String fishBird() {
		return "fishBird.jsp";
	}
